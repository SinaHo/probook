...
import { Crm as CrmData } from "./../../../admin/crm/api/CrmData";
                  ...
                  <Grid item xs={2}>
                    <IconButton
                      onClick={() => {
                        ...
                        this.props.history.push("/user/crm/" + item._id);
                      }}
                    >
                      <ExpandMore />
                    </IconButton>
...
Topic = withTracker(props => {
  Meteor.subscribe(`crm.user.getCreateds`);
  let Crm = CrmData.find({}).fetch();
  return { Crm };
})(Topic);
